...
Opening draft from the founders (user + Fabrizio, 50/50). This is living doctrine — sharpen it when reality teaches us, don't inflate it into HR poetry.
...
1. **Smallest honest slice.** Prove the idea with the least product that could possibly work — no fake scaffolding.
2. **Boring until it hurts.** Prefer proven tools. Clever tech needs a concrete reason, not a vibe.
3. **Readable > clever.** Optimize for the next human (including future us) at 1am.
...
| Person | Quirk |
| --- | --- |
| Dex | Sandwich ratings (1–10) on technical approaches |
| Maeve | Names the "villain" each feature defeats |
| Cal | B-movie horror titles for bugs |

These are personality, not process. If the bit gets in the way of clarity, drop the bit for that message.
